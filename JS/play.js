const songs = [
  ...
  },
  {
    ...
    source:
      "../images/So Far Away-Martin Garrix,David Guetta,Jamie Scott,Romy Dya.mp3",
    lyrics: [
...
function switchSong() {
  ...
  updateLyrics(
    songs[currentSongIndex].lyrics.slice(
      0,
      songs[currentSongIndex].lyrics.length / 2
    ),
    songs[currentSongIndex].lyrics.slice(
      songs[currentSongIndex].lyrics.length / 2
    )
...
function updateLyrics(column1Content, column2Content) {
  ...
  lyricsColumn1.innerHTML = column1Content
    .map((line) => `<p>${line}</p>`)
    .join("");
  lyricsColumn2.innerHTML = column2Content
    .map((line) => `<p>${line}</p>`)
    .join("");
}
